...
int pre_value( char *record)
{
  ...
  else if(compareStrings(string, "push")>0) 
  {
    ...
    if (vnr>-1)
    {
      /* split the value[vnr].string in separate strings at ' ' occurences and write them to the stack */
...
int getBodyParameters( char *record, char *name, char *ori, char ***cori,  char ***edge, int offset )
{
  ...
  if(compareStrings(ori, "ADD")==3)
  {
    ...
    for(k=0; k<=j; k++)
    {
      ...
      if(cori_[k]==NULL) { printf("ERROR: realloc failed in getSurfParameters()\n"); return(0); }
      cori_[k][0]=body[bodyNr].o[k];
      cori_[k][1]='\0';
      edge_[k]=(char *)malloc((int)(strlen(surf[body[bodyNr].s[k]].name)+1)*sizeof(char));
      if(edge_[k]==NULL) { printf("ERROR: realloc failed in getSurfParameters()\n"); return(0); }
  ...
  do
  {
    ...
    if(cori_[j]==NULL) { printf("ERROR: realloc failed in getSurfParameters()\n"); return(0); }
    cori_[j][0]=record[i];
    cori_[j][1]='\0';
    ...
    if(edge_[j]==NULL) { printf("ERROR: realloc failed in getSurfParameters()\n"); return(0); }

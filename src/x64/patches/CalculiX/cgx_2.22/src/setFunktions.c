...
typedef struct {
  ...
  FILE *handle;
  int   stopped;       /* if reading is interrupted:1 else: 0 */
  int   addFlag;       /* 1: new entity names */
  fpos_t *filepntr;    /* points to the actual file pos.  (fgetpos), unused */
  off_t byte_offset;    /* position after the stop command, unused */   
}CommandFile;
...
typedef struct{
    int flag;              // whileFlag: 
                           // 0 do not not execute
                           // 1 execute command
                           // 2 in a while loop, execute command
    ...
    // command file:
    char  filename[MAX_LINE_LENGTH];  // command file name
    FILE *handle;                     // command file handle
    fpos_t *filepntr;                 // points to the actual file pos.  (fgetpos), unused
    off_t byte_offset;                 // position after the stop command 
    int addFlag;
    int gtolFlag;
}whileStack;
...
int stack_ptr=-1;                     // active while loop
int nwhiles=0;                        // allocated while loops
whileStack *stack=NULL;               /* stores informations of while loops */
...
extern int       pixPerCharx[];
extern int       pixPerChary[];
...
int seti( int setNr, char *type, int sets, char dat[MAX_PARAM_PER_RECORD][MAX_LINE_LENGTH])
{
  ...
  if(sets>100) { printf(" ERROR, too many sets\n"); return(0); }
  j=0;
...
{
  int i,j,e,n;
  int etet, tets=0, extrapolflag=-1, includedFlag=0;
...
int pre_while(char *type, char **ptr_string, int *na, int *nb, FILE *handle1, int *addFlag, int *gtolFlag, int wFlag, int ifFlag)
{
  ...
  //printf("pre_while: type:%s str:%s nab:%d %d addf:%d gtolf:%d ",type,*ptr_string,*na,*nb,*addFlag,*gtolFlag);
  //printf("pre_while: whileFlag:%d\n",wFlag);
  
  if(compare( type,"WHILE",2)==2 )
  {
    //printf("pre_while: %s stackptr:%d\n",type,stack_ptr);

    sscanf(*ptr_string, "%*s %s %s %s", arg1, operator, arg2);
    //printf("pre_while:  args:%s %s  operator:%s\n",arg1,arg2,operator);

    i=getValuNr(arg1);
    if (i>-1) strcpy(arg1,value[i].string );
    i=getValuNr(arg2);
    if (i>-1) strcpy(arg2,value[i].string );
    //printf("pre_while:  args after substitution:%s %s  operator:%s\n",arg1,arg2,operator);
    //printf("while:  args after substitution:%f %f\n",atof(arg1),atof(arg2));

    if((operator[0]=='e')&&(operator[1]=='q')) { if(compareStrings(arg1,arg2)>0) flag=2; else flag=0; }
    else if((operator[0]=='n')&&(operator[1]=='e')) { if(compareStrings(arg1,arg2)<1) flag=2; else flag=0; }
    else if((operator[0]=='=')&&(operator[1]=='=')) { if(atof(arg1)==atof(arg2)) flag=2; else flag=0; }
    else if((operator[0]=='!')&&(operator[1]=='=')) { if(atof(arg1)!=atof(arg2)) flag=2; else flag=0; }
    else if(operator[0]=='>') { if(atof(arg1)>atof(arg2)) flag=2; else flag=0; }
    else if(operator[0]=='<') { if(atof(arg1)<atof(arg2)) flag=2; else flag=0; }
    else { printf("ERROR: operator not known:%s\n", operator); flag=1; } 

    //printf("pre_while: args:%s %s\n",arg1,arg2);
    //printf("pre_while: nwhiles:%d stackptr:%d flag:%d wflag:%d\n", nwhiles, stack_ptr, flag, wFlag);
    if(flag==1) return(1);  // error or strings to compare are not equal

    if((stack_ptr<0)||(wFlag==-2)) // create a new while-stack as long as the commands have to be saved
    {
      if ((stack = (whileStack *)realloc( (whileStack *)stack, (nwhiles+1)*sizeof(whileStack)) ) == NULL )
      { printf("\n\nERROR: realloc failure, whileStack\n\n"); return(1); }

      if((flag==2)&&(wFlag>=-2)) stack[nwhiles].flag=2;  // repeat and execute
      ...
      if(stack_ptr>-1)
      {
        ...
        //printf("pre_while: An open while (stack_ptr:%d) will reference this while:%d in a sequencial order:%d\n",stack_ptr, nwhiles, stack[stack_ptr].whiles);
      ...
      stack[nwhiles].addFlag=  *addFlag;
      stack[nwhiles].gtolFlag= *gtolFlag;
      stack_ptr=nwhiles;
      
      // store the current command file data
      strcpy(stack[stack_ptr].filename, commandFile[cur_commandFile].name);
      stack[stack_ptr].handle = commandFile[cur_commandFile].handle;
      if( (stack[stack_ptr].filepntr=(fpos_t *)malloc(1*sizeof(fpos_t))) == NULL ) printf(" ERROR: malloc failed for while\n");
      //if(fgetpos( stack[stack_ptr].handle, (fpos_t *)stack[stack_ptr].filepntr)!=0) { printf("error in fgetpos");  return(-1); }
      stack[stack_ptr].byte_offset=ftello(stack[stack_ptr].handle);

      //printf("pre_while: new while, stack_ptr:%d from file:%s handle:%x\n", stack_ptr, stack[stack_ptr].filename,stack[stack_ptr].handle);
  ...
  {
    //printf("pre_while:%s stackptr:%d stack[stack_ptr].flag:%d ifFlag:%d\n",type,stack_ptr, stack[stack_ptr].flag, ifFlag);

    // if the while was skipped from the beginning continue executing
    if((stack[stack_ptr].flag == 0))
    {
      // for serial whiles the stack_ptr has to be decreased by the amount of serial whiles
      stack[stack_ptr].lineptr=0;
      stack_ptr = stack[stack_ptr].prev_while;
      //printf("pre_while:  changed stack_ptr:%d\n", stack_ptr);
      if(stack_ptr>=0)
    ...
    else
    {
      ...
      if (i>-1) strcpy(arg2,value[i].string );
      //printf("pre_while:  args after substitution:%s %s  operator:%s\n",arg1,arg2,operator);
      //printf("endwhile:  args after substitution:%f %f\n",atof(arg1),atof(arg2));
  
      if((operator[0]=='e')&&(operator[1]=='q')) { if(compareStrings(arg1,arg2)>0) flag=2; else flag=1; }
      else if((operator[0]=='n')&&(operator[1]=='e')) { if(compareStrings(arg1,arg2)<1) flag=2; else flag=1; }
      else if((operator[0]=='=')&&(operator[1]=='=')) { if(atof(arg1)==atof(arg2)) flag=2; else flag=1; }
      else if((operator[0]=='!')&&(operator[1]=='=')) { if(atof(arg1)!=atof(arg2)) flag=2; else flag=1; }
      else if(operator[0]=='>') { if(atof(arg1)>atof(arg2)) flag=2; else flag=1; }
      else if(operator[0]=='<') { if(atof(arg1)<atof(arg2)) flag=2; else flag=1; }
      else { printf("ERROR: operator not known:%s\n", operator); flag=1; } 

      stack[stack_ptr].flag=flag;

      //printf("pre_while: flag:%d\n", flag);

      // if finished
      if((flag==1)||(ifFlag==0))
      {
	// delete the current while
	i=stack_ptr;
	...
	stack_ptr = stack[stack_ptr].prev_while;
	//printf("pre_while: changed stack_ptr:%d\n", stack_ptr);
        //if(stack_ptr>-1) printf("pre_while:    new_flag:%d\n", stack[stack_ptr].flag);
      ...
      //printf("pre_while:  re-read from file:%s lineptr:%d handle:%x\n",stack[stack_ptr].filename, stack[stack_ptr].lineptr,stack[stack_ptr].handle);
      //printf("pre_while:   command from stack: %s\n", stack[stack_ptr].string[stack[stack_ptr].lineptr]);

      //if( fsetpos( stack[stack_ptr].handle, (fpos_t *)stack[stack_ptr].filepntr)!=0) { printf("\n\nerror in fsetpos\n\n"); return(-1); }
      if( fseeko( stack[stack_ptr].handle, stack[stack_ptr].byte_offset, SEEK_SET)) { printf("\n\nerror in fseeko\n\n"); return(-1); }
      *nb=getCommandLine(stack[stack_ptr].handle, ptr_string);
      if ( *nb > 0)
      {
        type[0]=0;
        *na=0; while((*ptr_string)[*na]==' ') {  (*na)++; }
        *na+= sword( *ptr_string, type);
 	handle1=stack[stack_ptr].handle;
        *addFlag=stack[stack_ptr].addFlag;
        *gtolFlag=stack[stack_ptr].gtolFlag;
        //printf("pre_while: command           : %s\n", *ptr_string);
      }
      else return(2);
      //else { printf(" ERROR: Command not found. This should never happen. Please talk to the programmer.\n"); exit(0); }
      for(j=0;j<strlen(type); j++) type[j]=toupper(type[j]);
      return(2);
    }
  }

  //printf("pre_while:  return 0\n");
  return(0);
}
...
void pre_move( char *record)
{
  ...
  char setname[MAX_LINE_LENGTH];
  char type[MAX_LINE_LENGTH], command[MAX_LINE_LENGTH];  Nodes *bufn;
  char addDispFlagLocal=0;

  length = sword( record, setname );
  strcpy(command,&record[length]);
  ...
  if(set[nr].anz_p>0)
  {
    ...
    if(transform( command, set[nr].anz_p, bufn )<0) return;
  ...
  if(set[nr].anz_n>0)
  {
    ...
    if ( transform( command, set[nr].anz_n, bufn) <0) return;
    for (i=0; i<set[nr].anz_n; i++)
    {
...
int brecord( char *rec_str, char dat[MAX_PARAM_IN_BRECORD][MAX_LINE_LENGTH])
{
  ...
  for(i=0; i<=length; i++)
  {
    ...
    if((rec_str[i]==' ')&&(letter))
    {
      ...
      nextarg++;
      letter=0;
      if(nextarg>=MAX_PARAM_IN_BRECORD) { printf("ERROR: too many parameters in line:\n%s\n\n", rec_str); exit(1); }
    }
    else
    {
...
void compileTextString(Texts *ntext, int nr)
{
  char *buffer;
  if(ntext[nr].text!=(char *)NULL)
  {
    if((buffer=(char *)malloc((strlen(ntext[nr].text)+MAX_LINE_LENGTH)*sizeof(char))) == NULL)
      printf("\n ERROR: malloc failed\n");
  }
  else
  {
    if((buffer=(char *)malloc((MAX_LINE_LENGTH)*sizeof(char))) == NULL)
      printf("\n ERROR: malloc failed\n");
  }
  buffer[0]=0;
  if((ntext[nr].text)&&(ntext[nr].tFlag)&&(ntext[nr].pFlag==1)) sprintf(buffer,"%s ",ntext[nr].text);
  if((ntext[nr].nFlag)&&(ntext[nr].vFlag)&&(anz->l))
  {
    switch(ntext[nr].fFlag)
    {
    case 0:
       sprintf(&buffer[strlen(buffer)],"%d %6.2le", ntext[nr].node_nr, lcase[cur_lc].dat[cur_entity][ntext[nr].node_nr]);
      break;
    case 1:
       sprintf(&buffer[strlen(buffer)],"%d %.2f", ntext[nr].node_nr, lcase[cur_lc].dat[cur_entity][ntext[nr].node_nr]);
      break;
    case 2:
      sprintf(&buffer[strlen(buffer)],"%d %d", ntext[nr].node_nr, (int)lcase[cur_lc].dat[cur_entity][ntext[nr].node_nr]);
      break;
      printf("ERROR case:%d\n", ntext[nr].fFlag);
    }
  }
  else if(ntext[nr].nFlag) sprintf(&buffer[strlen(buffer)],"%d", ntext[nr].node_nr);
  else if((ntext[nr].vFlag)&&(anz->l))
  {
    switch(ntext[nr].fFlag)
    {
    case 0:
       sprintf(&buffer[strlen(buffer)],"%6.2le", lcase[cur_lc].dat[cur_entity][ntext[nr].node_nr]);
      break;
    case 1:
       sprintf(&buffer[strlen(buffer)],"%.2f", lcase[cur_lc].dat[cur_entity][ntext[nr].node_nr]);
      break;
    case 2:
      sprintf(&buffer[strlen(buffer)],"%d", (int)lcase[cur_lc].dat[cur_entity][ntext[nr].node_nr]);
      break;
      printf("ERROR case:%d\n", ntext[nr].fFlag);
    }
  }
  if((ntext[nr].text)&&(ntext[nr].tFlag)&&(ntext[nr].pFlag==0)) sprintf(&buffer[strlen(buffer)]," %s",ntext[nr].text);

  if((ntext[nr].string=(char *)realloc(ntext[nr].string,(strlen(buffer)+1)*sizeof(char))) == NULL)
    printf("\n ERROR: realloc failed\n");
  strcpy(ntext[nr].string,buffer);
  free(buffer);
}
...
void placeTxt(char *string)
{
  int   i,j, k, n=0, nr;
  double x=1,y=0, tdy;
  int args, ibuf, setNr=-1, dx,dy, pix, sidebound=0;
  GLint    viewport[4];
  GLdouble mvmatrix[16], projmatrix[16];
  char *text=(char *)NULL, name[MAX_LINE_LENGTH], dat[3][MAX_LINE_LENGTH];
  double tymin=MAX_FLOAT;
  static GLdouble wx, wy, wz;  /*  returned window x, y, z coords  */
  static int flag;
  static Rsort *rsort=NULL;

  dat[0][0]=dat[1][0]=dat[2][0]=0;
  ...
  if(n<1)
  {
    ...
    {
      delSet("-txt");
      if( (setNr=pre_seta("-txt", "i", 0)) <0) return;
      for(i=0; i<anz->t; i++)
      {
        if(ntext[i].node_nr>0) seta(setNr,"n",ntext[i].node_nr);
      }
    }
    else  
    {
  ...
  {
    delSet("-txt");
    if( (setNr=pre_seta("-txt", "n", name)) <0) return;
  }
    
  if(args==1) { dx=0; dy=0; k=-1; }
  else if(checkIfNumber(dat[0]))
  {
    x=atof(dat[0]);
    if(checkIfNumber(dat[0])) { y=atof(dat[1]); } else y=0;
    k=-1; dx=x*width_w1;
    dy=y*height_w1;
  }
  ...
  // if the set is not given, create a text only for the given nodenr n
  if(setNr<0)
  {
  ...
  skipSet:;
    ...
    if(text!=(char *)NULL)
    {
      if((ntext[nr].text=(char *)realloc(ntext[nr].text,(strlen(text)+1)*sizeof(char))) == NULL)
        printf("\n ERROR: realloc failed\n");
      strcpy(ntext[nr].text,text);
    }
    //printf(" nr=%d ntext[i].node_nr:%d ntext[t].text:%x\n",nr,ntext[nr].node_nr, ntext[nr].text);
    ntext[nr].tFlag=1;
    if(k>-1)
    {
      for(j=0; j<strlen(dat[k]); j++)
      {
        ...
        else if(dat[k][j]=='l') sidebound=1;
        else if(dat[k][j]=='r') sidebound=2;
      }
    }
    compileTextString(ntext, nr);
    ...
    if((dx)||(dy)||(sidebound))
    {
      ...
      if(sidebound==1)
      {
        //  place the texts on the left border
        ntext[nr].tx= 1;
	if(x==0.)  ntext[nr].ty=(viewport[3]-wy);
        else ntext[nr].ty= -y/x*(-wx)+(viewport[3]-wy);
        tymin=dmin(tymin,ntext[nr].ty);
      }
      else if(sidebound==2)
      {
        //  place the texts on the right border
        ntext[nr].tx=viewport[2]-1;
	pix=strlen(ntext[nr].string);
        for(j=0; j<pix; j++) ntext[nr].tx-=glutBitmapWidth(glut_font[draw_font],ntext[nr].string[j]);
	if(x==0.)  ntext[nr].ty=(viewport[3]-wy);
	else ntext[nr].ty= y/x*(-wx)+(viewport[3]-wy);
        tymin=dmin(tymin,ntext[nr].ty);
      }
	...
	tymin=0.;
      ...
      //printf(" tymin:%lf ty:%lf\n",tymin,ntext[nr].ty);
    }
  }
  free(text);

  
  // prevent overlapping text positions
  // sort them according to their y coordinate
  if ( (rsort = (Rsort *)malloc( (anz->t+1) * sizeof(Rsort))) == NULL )
    printf("ERROR: realloc failed: Rsort\n\n" ); 
  nr=0;
  for(i=0; i<anz->t; i++)
  {
    if(ntext[i].node_nr>0)
    {
      //printf(" tymin:%lf ntext[i].ty:%lf\n",tymin, ntext[i].ty);
      if((sidebound)&&((tymin<0.)||(x==0.))) ntext[i].ty+=-tymin+pixPerChary[draw_font];
      //printf(" ntext[i].ty:%lf\n", ntext[i].ty);

      rsort[nr].r=ntext[i].ty;
      rsort[nr].i=i;
      nr++;
    }
  }
  qsort( rsort, nr, sizeof(Rsort), (void *)compareRsort );
 repeat:;
  for(i=1; i<nr; i++)
  {
    tdy=ntext[rsort[i-1].i].ty-ntext[rsort[i].i].ty;
    //printf(" %d %d : %lf draw_font:%d %d\n", ntext[rsort[i-1].i].node_nr, ntext[rsort[i].i].node_nr, tdy, draw_font, pixPerChary[draw_font]);
    if(abs(tdy)<pixPerChary[draw_font])
    {
      // check if the strings overlap in x direction
      // which is left?
      /*
      pix=0;
      if(ntext[rsort[i-1].i].tx<ntext[rsort[i].i].tx)
      {
	k=strlen(ntext[rsort[i-1].i].string);
        for(j=0; j<k; j++) pix+=glutBitmapWidth(glut_font[draw_font],ntext[rsort[i-1].i].string[j]);
	if(ntext[rsort[i-1].i].tx+pix>ntext[rsort[i].i].tx) continue;
      }
      else
      {
	k=strlen(ntext[rsort[i].i].string);
        for(j=0; j<k; j++) pix+=glutBitmapWidth(glut_font[draw_font],ntext[rsort[i].i].string[j]);
	if(ntext[rsort[i].i].tx+pix>ntext[rsort[i-1].i].tx) continue;
      }
      */
      //printf(" nodes to close, pos will be corrected\n");
      if(tdy<0.) ntext[rsort[i].i].ty+=pixPerChary[draw_font];
      else ntext[rsort[i-1].i].ty+=pixPerChary[draw_font];
      goto repeat;
    }
  }
}
...
int commandoInterpreter( char *type, char **ptr_string, int na, int nb, FILE *handle1, int addFlag, int *gtolFlag )
{
  int   i,j,k,l,n; 
  int   lc,nr,ibuf=0;
  char  *string;
  ...
  /* whileFlag=0 condition was not fulfilled, skip the commands
     whileFlag=1 execute commands
     whileFlag=2 in a loop, execute commands
  */
  //printf("commandoInterpreter: whileFlag_in:%d string:%s\n",whileFlag, string);
  if(compareStrings(type, "WHILE")>0)
  {
    whileFlag=pre_while(type, &string, &na, &nb, handle1, &addFlag, gtolFlag, -whileFlag,ifFlag);
    *ptr_string=string;
    goto checkForError;
  }
  if(compareStrings(type, "ENDWHILE")>0)
  {
    whileFlag=pre_while(type, &string, &na, &nb, handle1, &addFlag, gtolFlag, -whileFlag,ifFlag);
    *ptr_string=string;
    if(nb<=0) goto checkForError;
  }
  //printf("commandoInterpreter: whileFlag_out:%d na:%d string:%s\n",whileFlag,na,string);
  ...
  if (compare(type, "VALU", 4)!=4)
  {
    ...
    for(i=0; i<args; i++)
    {
      ...
      if (j>-1)
      {
        //printf ("commandoInterpreter: param:%s replaced by value:%s\n", value[j].name, value[j].string );
        strcpy(dat[i],value[j].string );
      }
      sprintf(&string[strlen(string)]," %s",dat[i]);
    }
    //printf("commandoInterpreter: replaced parameters by defined values: %s\n", string);
  }
  ...
  if (compareStrings(type, "ENDWHILE")>0);  // just ignore
  else if (compareStrings(type, "ALLOW_SYS")>0);
  ...
  else if (compareStrings(type, "CONT")>0)
  {
	...
	if((commandFile[stopped_commandFile].handle!=NULL)&&(commandFile[stopped_commandFile].stopped))
	  ...
	  //printf("new call to readfbd \n");
          readfbd( 0, 0);
	  //printf("readfbd finished\n");
          return(0);
	}
    }
  ...
  else if (compareStrings(type, "COL")>0)
  {
    ...
    for(i=0; i<entitycols; i++)
    {
      if(compareStrings(dat[0],entitycol[i].name)>0)
      {
        ...
        length=sscanf(&string[na+1], "%s %lf %lf %lf %lf", entitycol[i].name, &x, &y, &z, &lfbuf );
	entitycol[i].r=x/lfbuf;
	entitycol[i].g=y/lfbuf;
	entitycol[i].b=z/lfbuf;
	goto checkForError;
      }
    }
    entitycols++;
  ...
  {
    buffer[0]=0;
    length = sscanf(&string[na+1], "%d %s %s", &nr, name, buffer);
    if(length<2) { printf(" wrong syntax\n"); goto checkForError; }
    operateAlias( name, "se" );
    setNr=getSetNr(name);
    if (setNr<0)
    {
      printf (" ERROR in grpa: set:%s does not exist\n", name);
      goto checkForError;
    }
    if (buffer[0]=='d')
    {
      sprintf(buffer,"%s_group%d.dyn",name,nr);
      handle=fopen(buffer,"w");
      if(handle==NULL) goto checkForError;
      fprintf(handle,"*PART\n");
      fprintf(handle,"%s\n",name);
      fprintf(handle,"$#   pid   secid     mid\n");
      fprintf(handle,"%8d       0       0\n",nr);
      fclose(handle);
    }
    for (i=0; i<set[setNr].anz_e; i++)
    {
      e_enqire[set[setNr].elem[i]].group=nr;
    }
  }
  ...
  else if (compareStrings(type, "TEST")>0)
  {
	...
	strcpy(parameter[0],"TRUE");
	   ...
	   //na is now the nr of inside nodes and not a nodenr
  ...
  else if (compareStrings(type, "THRS")>0)
  {
    ...
    sscanf(&string[na+1] , "%s %s %s %s", name, xbuf, format, comm);
    ...
    sprintf(buffer,"%s %s rec _ _ _ 0. %s %s",name, specialset->thrs, format, xbuf );
...
{
  FILE      *handle1=NULL;
  int       result=0, length, na, gtolFlag=0, updStackHandle=0;
  char type[MAX_LINE_LENGTH];
  ...
  //printf("readfbd: backgroundFlag:%d inpformat:%d inpformatbuffer:%d\n",backgroundFlag,inpformat, inpformatbuffer);
  ...
  if((datin==0)&&(stopped_commandFile>-1))  // restart reading
  {
    if(commandFile[stopped_commandFile].stopped==1)
    {
      ...
      //printf("readfbd: file:%s set handle to handle:%x\n",  commandFile[cur_commandFile].name, handle1);
      commandFile[cur_commandFile].stopped=0;
      addFlag=commandFile[cur_commandFile].addFlag;

      // close and re-open the file to catch updates
      // and eventually update the while loop:
      if((stack_ptr>-1)&&(stack[stack_ptr].handle==handle1)) updStackHandle=1;
      offset=ftello(handle1);
      fclose(handle1);
	...
	return(1);
      ...
      if( fseeko( handle1, offset, SEEK_SET)) { printf("\n\nerror in fseeko\n\n"); return(-1); }
      ...
      // update also the while loop:
      if(updStackHandle)
      {
	//printf("update also the while loop handle\n");
	stack[stack_ptr].handle = handle1;
      }
      //printf("readfbd: reopened file:%s  new handle:%x  new handle:%x\n",  commandFile[cur_commandFile].name, handle1, handle2);
    }
  ...
  else
  {
    ...
    commandFiles++;
    //printf("readfbd: open cur_commandFile %d files %d\n", cur_commandFile, commandFiles);
  ...
  else // it's not a "STOP" command, file wil be closed
  {
    ...
    {
      stopped_commandFile--;
      //printf("readfbd: next stopped_commandFile nr:%d\n",stopped_commandFile);
      if((commandFile[stopped_commandFile].handle!=NULL)&&(commandFile[stopped_commandFile].stopped))
      {
        printf(" CONTINUE to read stopped_commandFile:%s\n", commandFile[stopped_commandFile].name);
        readfbd( 0, 0);
        return(0);
      }
    }
...
int pre_read( char *record )
{
  ...
  else if((compare( ext, "frd", 3)== 3)||(compare( param1, "ng", 2)== 2)||(compare( ext, "stl", 3)== 3)||(compare( ext, "inp", 3)== 3)||(compare( param1, "inp", 3)== 3)||(compare( param1, "foam", 4)== 4)||(compare( param1, "vtk", 3)== 3)) 
  {
    ...
    for(i=0; i<apre->sets; i++)
    {
      ...
      // if the set does not exist then a simple copy is NOT possible since non existing entities must be detected
      if(nset<0)
      {
	...
	{
          for (j=0; j<spre[i].anz_n; j++) { seta(nset,"n",spre[i].node[j]); }
	  /*
	...
	{
          for (j=0; j<spre[i].anz_e; j++) { seta(nset,"e",spre[i].elem[j]); }
	  /*
...
int completeSet_frame( int setNr, char type )
{
  int i,j, m, n, p, l, c, s;
    ...
    /* cyrcle through all sets and add all entities except nodes and elements */
    for (i=0; i<set[setNr].anz_se; i++)
    {
      s= set[setNr].set[i];
      if((!type)||(type=='f'))
    ...
    /* ignore the referenced nodes and elements */
    //set[setNr].anz_n=0;
    //set[setNr].anz_e=0;

    /* cyrcle again through all sets and add nodes and elements */
    for (i=0; i<set[setNr].anz_se; i++)
    {
      s= set[setNr].set[i];
      if((!type)||(type=='n'))
...
void pre_write( char *record )
{
  ...
  operateAlias( setname, "se" );
  if(getSetNr(setname)<0) { printf(" ERROR: Set:%s does not exist\n",setname); return; }
  printf (" please wait for 'ready'\n");
...
*/
  ...
  else if (compare( format, "dyn", 3)== 3)
  {
    if (length>2)
    {
      if (compare( type, "nam", 3)== 3)
      {
        sendNames( setname, format, anz, node, e_enqire );
      }
      else if (compare( type, "sur", 3)== 3)
...
#define DT_KELVIN_TO_C -273.15
  ...
  else if (compare( format, "frd", 3) == 3)
  {
        ...
        if(lcase2[0].ncomps)
	{
      ...
      else
      {
       ...
       {
        /* check if the data of the specified lcase (Dataset) are already available */
	/* but do that only if (new) nodes exist which have not received their dataset values. */
	/* if possible load the data in write2frd() since they will be freed there after writing */
	 //printf("copiedNodeSets[0].sets:%d set_qcut:%d\n",copiedNodeSets[0].sets, set_qcut);
	 if(((set_qcut>-1)||(copiedNodeSets[0].sets))&&(!lcase[lc].loaded))
        {
          if( pre_readfrdblock(copiedNodeSets , lc, anz, node, lcase )==-1) 
          {
            printf(" ERROR in nodalDataset: Could not read data for Dataset:%d\n", lc+1); 
  ...
  {
    anz2->l = 0;
    write2dyna( prognam, anz2, node2, elem2, lcase);
  }
  else
    printf (" ERROR: Format:%s not recognized",format);

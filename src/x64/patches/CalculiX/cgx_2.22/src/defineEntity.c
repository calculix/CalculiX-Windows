...
void delNod( int anzn, int *index ) 
{
  ...
  for (j=0; j<anzn; j++)
  {
    // TBD: it might be necessary to skip nodes <0 and >anz->nmax
    ...
    // TBD: eventually remove the node from lines, surfs etc.
    // due to efficieny reasons it is better to use the pflag to avoid
    // using this node.
    node[buf[j]].pflag=-1;
  }
  free(buf);
...
int nurs( char *string, int addFlag )
{
  ...
  else if(( compare( action, "END", 3) == 3 ) || ( compare( action, "end", 3) == 3 ))
  {
    ...
    */
    ...
    if( shape_i( nurbs[nr].name, 4, nr, 0, 0, 0, 0, 0, 0)<0 )
      printf("ERROR: shape could not be created\n");
#endif
  }
  else

...
int splitLine(int l, int *l_nr, int ps_nr)
{
  ...
  double ltot,l1,sp1,sp2,lv10,lp01,lp10,nodeSpacing;
...
{
  int  c,e,i,j=0,k,n,nn,m,p,ll,l,d1,d2,ss,s, setNr, trgtNr, trkNr, mode, ibuf=0, orient=0;
  char setname[MAX_LINE_LENGTH], targetname[MAX_LINE_LENGTH], name[MAX_LINE_LENGTH], param[MAX_LINE_LENGTH];
  double pl[2][3],ps[3][3], pc[3], psplt[10][3], vproj[3], dist=0., dist2=0., distn=0., lp0p1, lps;
  ...
  if (setNr<0)
  {
    ...
    if( (setdep=pre_seta( specialset->dep, "l", setname )) <0 ) return;
  }
  else
  {
    ...
    for (ll=0; ll<set[setdep].anz_e; ll++)
    {
	  ...
	  vn_av[2]=(pl[1][2]+pl[0][2])*.5;
          ...
          for( nn=0; nn<n_closest_nodes; nn++)
          {
            ...
            v_scal( &g, eg, p01 );
            v_add( pl[0], p01, psplt );
            // projpl[0] and pl[1] onto en and generate new points there
            v_result( psplt, pl[0], p01 );
            g=v_sprod(p01,en);
            v_scal(&g,en, p01);
            v_add(psplt,p01,pp1);
            v_result( psplt, pl[1], p01 );
            g=v_sprod(p01,en);
            v_scal(&g,en, p01);
            v_add(psplt,p01,pp2);
 ...
 nextEdge:;
	  ...
	  //printf("ip:%d nip:%d n:%d dist2:%e dist:%e dist2*dist:%e o:%d\n",p,line[l].nip,n, dist2, dist, dist2*dist, orient);
		...
		seta(setSurf, "s", set[setMesh].surf[i]);
	      ...
	      //if(line[l].typ=='s') convertLine( l,line[l].div-1);
              if(splitLine(l, l_nr, ps_nr)==-1) return;
  ...
  /* go over all surfs which should be splitted and create the split-points  */
  if(sum_splitbuf>1) for (ss=0; ss<set[setNr].anz_s; ss++)
  {
    s=set[setNr].surf[ss];
    ...
    // - search and store the splitbuf[].p which relate to the actual surface in linep
      ...
      for(i=0; i<sum_splitbuf; i++)
      {
		...
		printf("\n\n ERROR: malloc failure\n\n" );
  	      ...
  	      if(splitbuf[i].l==ll)
	      {
	        // is that point already stored?
	        for(k=0; k<sum_linep; k++) if(linep[k]==splitbuf[i].p) goto nextline;
                //printf ("found (%c) (%s), store p%d %s\n", lcmb[l].o[k], line[ll].name, sum_linep+1, point[splitbuf[i].p].name);
                if ( (linep = (int *)realloc((int *)linep, (sum_linep+1)*sizeof(int))) == NULL )
		  printf("\n\n ERROR: malloc failure\n\n" );
	        linep[sum_linep]=splitbuf[i].p;
                sum_linep++;
	...
	if(printFlag) printf("track starts at linep %s\n", point[linep[k]].name);
	p=-1;
	dist2=MAX_FLOAT;
	...
	if(p==-1)
	{
	  printf(" REMARK, no points between splitpoints found, create a straight line\n");
	  ...
	  continue;
        ...
        linep[k]=-1;
        if(printFlag) printf("add 1st pnt:%s\n",point[surfp[p]].name);
	...
	do
	{
  	  ...
  	  //if(n1==-1) printf("no more points, generate the line\n");
	    ...
	    if(mode==0)
	    {
  	      ibuf=((set[trkNr].anz_p-2)/2)*2;
	      if(ibuf<2) ibuf=2;
  	      if(set[trkNr].anz_p<2) { printf("ERROR only %d points in trk:%s\n",set[trkNr].anz_p,set[trkNr].name); return; }
              ...
              l=line_( name, point[set[trkNr].pnt[0]].name, point[set[trkNr].pnt[set[trkNr].anz_p-1]].name, set[trkNr].name, ibuf, 1 );
      ...
      if(printFlag) printf(" all lines generated for surf: %s\n\n",surf[s].name);
    }
    free(linep);
    free(surfp);
    linep=NULL;
  ...
  for(i=0; i<sum_surfbuf; i++)
  {
    ...
    // organize the surfbuf[i].l in a way that all lines between two splitpoints are grouped together
    // the connected lines will be stored in linebuf[sum_linebuf].l
    sum_linebuf=0;
    for(k=0; k<surfbuf[i].nl; k++)
    {
      ibuf=0;
      for(j=0; j<surf[s_nr[0]].nl; j++)
      {
	...
	if ( (linebuf[sum_linebuf].l = (int *)realloc((int *)linebuf[sum_linebuf].l, (linebuf[sum_linebuf].nl+1)*sizeof(int))) == NULL )
      ...
      {
	printf(" TBD:starts a chain of lines until a subsequent line has a common point with the outer surface lines\n");
      }
      else { printf(" ERROR: found an initial inner line which does not intersect with the outer surface lines %d\n",ibuf); }
    }
    
    for(k=0; k<sum_linebuf; k++) if(linebuf[k].nl==1)
    {
      if(s_nr[1]>-1)  // earliest at second loop k
      {
	...
	for(s=0; s<2; s++)
	{
	  ...
	  for(j=0; j<surf[s_nr[s]].nl; j++)
	  {
	    //compare the 1st lpnt with the pnts of splitline, both splitpnts must be found
	    l=surf[s_nr[s]].l[j];
	  ...
	  if(ibuf>3) { s_nr[0]=s; ss=1; }
	}
	if(!ss) { printf(" ERROR: No surf found for line:%s\n",line[linebuf[k].l[0]].name); return; }
      }
	    
      // split the surface
      s_nr[1]=splitSurf( 1, &linebuf[k].l[0], s_nr[0], 0 );
      if(s_nr[1]<0)
      {
    ...
    else { printf(" WARNING: Found a chain of lines (TBD)\n"); continue; }
  }

  // try to generate a surface using the splitting lines
  mode=0;
  for(ii=0; ii<sum_surfbuf; ii++)
  {
    if(surfbuf[ii].nl==1)
    {
      if(mode>3) { mode=0; break; }
      l_nr[mode]=surfbuf[ii].l[0];
      mode++;
    }
  }
  if(mode==4)
  {
    getNewName( name, "s");
    ibuf=surface_i( name, '+', -1, (int)4, "++++", &l_nr[0], "llll");
    if(ibuf>-1)
    {
      surf[ibuf].eattr=-1;
      surf[ibuf].etyp=surf[surfbuf[0].s].etyp;
      seta(setSurf,"s",ibuf);
    }
  }
  
  // mesh again
  if((meshAgainFlag)&&(set[setSurf].anz_s))
  {
...
#define PROJ_LOOPS 9 // must be <10 since the value is used as an exponent to 10 to calc an int value
  ...
  double p01[3],eu[3],ev[3],en[3],eg[3],g,psplt[3],pp1[3];
  int  minj=0, anz_n=0;
  ...
  if((functionFlag)&&(!sum_tri))
  {
    ...
    //else printf(" ERROR: Transformation type %s not implemented for shapes (only 'nor'), please use a surface.\n", action);
  }
...
int zap(char *record)
{
  ...
  if (set[setNr].anz_b>0)
  {
	...
	gbod_i( body[b].name, 0, s, cori, edge );
...
void pre_eprop(char *setname)
{
  ...
  sprintf( lcase[anz->l].dataset_name,"");
...
int commandoInterpreter( char *type, char *string, int na, int nb, FILE *handle1, int addFlag, int *gtolFlag )
{
  ...
  */
  ...
  else if (compareStrings(type, "SEQL")>0)
  {
    ...
    if(k==0)
    {
      for(i=0; i<set[setNr].anz_l; i++) { line[set[setNr].line[i]].typ=' '; line[set[setNr].line[i]].trk=-1; }
      goto checkForError;
    }
    l=1;
    if(set[setNr].flag=='c') seto(set[setNr].name);
  ...
  else if (compare( format, "abq", 3)== 3)
  {
    if (length>2)
    {
      if (compare( type, "quadlin", 3)== 3)
        ...
        sendPressure( setname, format, anz, node, e_enqire, val[0], val[1], val[2], -1 );
